...
2. タスクの実行
   ...
   * **Supabase RLS設定**
     ...
     * INSERT: anon 用ポリシーは設けず、ジョブ作成はバックエンド（service_role）のみ。レート制限はAPI層で実施。
   ...
   * **Gemini API統合**
     ...
     * スタイルタイプに応じたプロンプトを動的に生成（`docs/specs/api-design.md`参照）
   ...
   * **スタイルタイプ定義**
     ...
     * 各スタイルに対応するプロンプトは`docs/specs/api-design.md`に定義済み
...
3. 品質管理と問題対応
   ...
   * APIエンドポイントでは以下のエラーコードとユーザーメッセージを使用すること（`docs/specs/api-design.md`参照）：
...
7. **プロジェクト設計ドキュメント**
   ...
   * `docs/specs/database-design.md` - データベース設計（テーブル定義、RLS設定、ストレージ設計）
   * `docs/specs/api-design.md` - API設計（エンドポイント定義、内部処理シーケンス、エラーハンドリング）
...
* ✅ **変更点が docs / Notion テンプレに記録されている**
  * API変更時は `docs/specs/api-design.md` を更新すること
  * データベース変更時は `docs/specs/database-design.md` を更新すること
