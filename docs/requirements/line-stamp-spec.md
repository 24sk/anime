...
| **Phase 2（サブスク上位プラン）** | ログイン＋サブスク契約者向けに **複数選択可（最大 40 個）** | 選択した複数文言それぞれを **AI で生成** | 複数枚＋メイン・タブ画像を ZIP で一括ダウンロード |

* 料金・レート制限の影響を抑えるため、Phase 1 では 1 ワード・1 回の AI 生成に限定する。待ち時間の設計（生成中表示など）を明確にする。
* Phase 2 では、ユーザーが 8 / 16 / 24 / 32 / 40 の中から生成枚数を選択可能とし、初期値は 8 枚とする（推奨セット `recommended_8` と整合）。
...
| **Phase 2（サブスク）** | スタンプ画像 | 選択した複数文言それぞれに対し、AI が生成。ファイル名は `01.png`, `02.png` …（最大 40 枚まで）。 |
| **Phase 2（サブスク）** | メイン・タブ画像 | 元画像を 240×240 / 96×74 にリサイズし、常に ZIP に含める（`main.png`, `tab.png`）。 |
| **Phase 2（サブスク）** | 配布形式 | 複数枚の PNG とメイン・タブ画像を **ZIP で一括ダウンロード**。 |

### 3.4 非機能要件

* **Phase 1:** 1 ワード・1 回の AI 生成のため、料金・レート制限の影響を最小化する。生成完了まで数十秒程度を想定し、**待ち時間の設計**（生成中表示・進捗やキャンセル等）を明確にする。
* **Phase 2（サブスク）:** 複数枚の AI 生成を行うため、同時生成枚数上限（最大 40 枚 / リクエスト）やレート制限・コスト管理を設ける。1 ユーザー（anon_session_id）あたりの日次上限は 40 枚 / 日を目安とし、IP アドレス単位でも 200 枚 / 日 程度の上限を併用する。
* **互換性:** LINE の仕様変更に合わせて、出力サイズ・ファイル名ルールを仕様書と実装で更新できるようにする。

### 3.5 失敗時の扱い・再試行ルール

* Phase 2 では、スタンプ生成が一部失敗した場合でも、**成功した画像は保持し、失敗した文言のみ再生成を試みる**。
* 各文言ごとに再試行回数の上限（例: 最大 2 回）を設け、上限に達した文言は「最終的に失敗」として扱う。それ以上の自動再生成は行わない。
* 再試行上限に達した文言については、UI 上で「このテキストでは生成できない可能性があります。文言を変更してお試しください。」等のメッセージを表示し、ユーザーが文言そのものを変更することで新たな試行ができるようにする。
* ZIP ダウンロードには、最終的に成功したスタンプ画像とメイン・タブ画像のみを含める（失敗した文言のスタンプ画像は ZIP に含まれない）。
...
| 文言選択 | プリセット（単語）から **1 ワードのみ** 選択。**自由入力**で 1 文言の指定も可能。チップまたはカードで単一選択 UI。セット・カテゴリタブは単語の単一選択として表示 | 複数選択可能。セット・カテゴリで一括選択を提供。「おすすめ8個」等を目立たせる。生成枚数は 8 / 16 / 24 / 32 / 40 から選択でき、初期値は 8。|
| プレビュー | **廃止**。代わりに「スタンプを生成」実行後、**AI 生成結果 1 枚**を表示する | 複数枚の生成結果をグリッド等で表示 |
| オプション | 対象外（1 枚のため） | 生成枚数選択（8 / 16 / 24 / 32 / 40、初期値 8）や注意書きなど。メイン画像・タブ画像は常に ZIP に含めるため、チェックボックスは不要。 |
...
* **Phase 2:** クライアントまたはサーバー側で **JSZip** 等を用い、複数枚の PNG を ZIP にまとめてダウンロード。ファイル名はスタンプ画像が `01.png` 〜（最大 40 枚）、メイン/タブ画像は `main.png`, `tab.png` とする。
