...
- [x] **共通レイアウト作成**
  - `layouts/default.vue` にヘッダー（ロゴ、使い方ガイドボタン、カラーモード）とメインコンテンツエリアを実装
...
- [x] **ヘッダーコンポーネント**
  - サービスロゴ（AniMe）
  - 使い方ガイドボタン（「使い方」クリックで Driver.js のインタラクティブツアーを開始。トップ画面の各要素を順にハイライトして案内）
  - 使用コンポーネント: `UHeader`、`UButton`、nuxt-driver.js（`useDriver()`）
- [x] **画像アップロードエリア**
